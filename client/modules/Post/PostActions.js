...
// po udanym zapisaniu na serwerze aktualizujemy store
...
export function thumbUpRequest(cuid, post) {
  return (dispatch) => {
    return callApi(`posts/${cuid}`, 'put', { post: { votesCount: post.votesCount } }).then(() => dispatch(thumbUp(cuid)));
  };
}

export function thumbDownRequest(cuid, post) {
  return (dispatch) => {
    return callApi(`posts/${cuid}`, 'put', { post: { votesCount: post.votesCount } }).then(() => dispatch(thumbDown(cuid)));
  };
}
